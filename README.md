...
- 맛집 정보 및 평점 (1-5점)
- 이미지 업로드 (최대 5장, GCS Presigned URL)
- 좋아요 및 조회수 관리
- 댓글 및 대댓글 (계층적 구조)
...
- `PostgreSQL 14` `Flyway` (데이터베이스 버전 관리)

**Cloud & Infrastructure**

- `Google Cloud Storage` (이미지 스토리지, V4 Signed URLs)
- `Compute Engine` (e2-micro VM, Container-Optimized OS)
- `Managed Instance Group` (자동 스케일링 및 롤링 업데이트)
- `Artifact Registry` (컨테이너 레지스트리)
- `Cloud SQL` (PostgreSQL, Private IP)
- `Cloud Load Balancing` (HTTP(S) Load Balancer)
- `VPC Network` (Public/Private Subnet 분리)
- `Cloud NAT` (Private Subnet 아웃바운드 인터넷 액세스)

**DevOps & CI/CD**

- `Docker` (Multi-stage 빌드, Container-Optimized OS)
...
- **인프라**: Google Cloud Platform 기반 자동 배포 (MIG, Cloud SQL, Load Balancing)
- **리전**: asia-northeast3 (Seoul, South Korea)
- **CI/CD**: GitHub Actions + Workload Identity Federation 기반 완전 자동화 파이프라인

### 📈 성과 지표

| 항목            | 성과                                  |
|---------------|-------------------------------------|
| **배포 자동화**    | MIG 기반 무중단 자동 배포 (롤링 업데이트)          |
| **코드 품질**     | SonarCloud 커버리지 80%+ 유지             |
| **테스트 코드**    | 약 15,000줄 (단위/통합/API 테스트)           |
| **아키텍처**      | 헥사고날 + DDD + Event-Driven 적용        |
| **이미지 시스템**   | GCS V4 Signed URL로 서버 부하 최소화        |
| **네트워크 보안**   | Private Subnet + Cloud NAT + IAP 적용 |
| **인증 보안**     | Workload Identity Federation (키리스)  |
...
└──────────┬───────────┘
           ...
           ▼
┌──────────────────────┐    ┌──────────────────────┐
│   Artifact Registry  │───▶│  Managed Instance    │
│ (Docker Images)      │    │  Group (MIG)         │
└──────────────────────┘    │  - Rolling Update    │
                            │  - Auto Healing      │
┌──────────────────────┐    │  - 2 VM Instances    │
│    SonarCloud        │    └──────────┬───────────┘
...
│  │  (PostgreSQL)    │    │  (Images/GCS)    │      │
│  │  Private IP Only │    │  V4 Signed URLs  │      │
│  └──────────────────┘    └──────────────────┘      │
└────────────────────────────────────────────────────┘

Security Features:
✓ Private Subnet (no public IPs on VMs)
✓ Cloud NAT (controlled outbound access)
✓ IAP (Identity-Aware Proxy for SSH)
✓ Workload Identity Federation (keyless auth)
✓ Private Services Connection (Cloud SQL)
```
