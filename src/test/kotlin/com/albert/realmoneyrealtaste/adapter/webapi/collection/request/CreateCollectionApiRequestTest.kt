...
class CreateCollectionApiRequestTest {
    ...
    fun `toCommand - success - converts visibility to privacy correctly`() {
        val publicRequest = CreateCollectionApiRequest(
            name = "공개 컬렉션",
            visibility = "PUBLIC"
        )
        val privateRequest = CreateCollectionApiRequest(
            name = "비공개 컬렉션",
            visibility = "PRIVATE"
        )
        val invalidRequest = CreateCollectionApiRequest(
            name = "잘못된 컬렉션",
            visibility = "INVALID"
        )
    ...
    fun `toCommand - success - handles empty cover image url`() {
        ...
        val requestWithBlank = CreateCollectionApiRequest(
            name = "컬렉션",
            coverImageUrl = "   "
        )
    ...
    fun `validation - success - case insensitive visibility conversion in toCommand`() {
        val mixedCaseRequest = CreateCollectionApiRequest(
            name = "컬렉션",
            visibility = "public"
        )
    ...
    private fun findViolationByProperty(
        violations: Set<ConstraintViolation<CreateCollectionApiRequest>>,
        propertyName: String,
    ): ConstraintViolation<CreateCollectionApiRequest>? {
